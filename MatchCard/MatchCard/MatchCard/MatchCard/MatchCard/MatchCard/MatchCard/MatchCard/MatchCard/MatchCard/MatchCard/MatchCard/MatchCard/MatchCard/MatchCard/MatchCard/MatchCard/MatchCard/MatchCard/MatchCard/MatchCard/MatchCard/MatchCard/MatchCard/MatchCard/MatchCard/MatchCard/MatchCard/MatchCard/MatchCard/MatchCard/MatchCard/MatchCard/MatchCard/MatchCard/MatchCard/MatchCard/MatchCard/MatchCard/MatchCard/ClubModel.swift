...
class ClubModel : PFObjectImaged, PFSubclassing {
    ...
    convenience init(latitude:Float, longitude:Float, name:String, teams:[TeamInClubModel]?=nil, players:[PlayerInClubModel]?=nil)
    {
        self.init()
        self.name = name
    ...
    @NSManaged var players : [PlayerInClubModel]?
}