...
class EasyTipView: UIView, Printable {
    ...
    */
    ...
    class func showAnimated(animated : Bool, forView view : UIView, withinSuperview superview : UIView?, text :  NSString, preferences: Preferences?, delegate : EasyTipViewDelegate?){
        ...
        EasyTipView.showAnimated(animated, forView: view, withinSuperview: superview, text: text, preferences: preferences, delegate: delegate, dismissAfterDelay: 0)
    }
    
    class func showAnimated(animated : Bool, forView view : UIView, withinSuperview superview : UIView?, text :  NSString, preferences: Preferences?, delegate : EasyTipViewDelegate?, dismissAfterDelay: Double){
        ...
        if dismissAfterDelay > 0 {
            Common.delay(dismissAfterDelay, closure: { () -> () in
                ev.handleTap()
            })
        }
    }
